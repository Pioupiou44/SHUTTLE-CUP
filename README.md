...
- Wizard de création de tournoi (jusqu'à 7 étapes selon le format)
- Génération automatique du planning selon 5 formats actifs (round-robin, knockout, double-elimination, pool+knockout, américano)
...
- Vue planning avec permutation de matchs, numéro de terrain modifiable
- Vue arbitrage plein écran avec saisie de score en temps réel
- Raccourcis clavier pour l'arbitrage
- Vue bracket knockout (avec avancement automatique des gagnants)
- Classements : général (round-robin), américano (points cumulés), Swiss (Buchholz), King of Court (victoires terrain 1)
- Génération dynamique des rondes suivantes (Swiss et King of Court — engines prêts, formats désactivés dans le wizard)
- Impression planning + classements + résultats (`/tournaments/:id/print`)
- Règles de scoring custom (éditeur dans Paramètres)
...
| `/tournaments/new` | Wizard création (jusqu'à 7 étapes) | ✅ |
| `/tournaments/:id` | Détail tournoi — planning, classement, poules, bracket, joueurs | ✅ |
| `/tournaments/:id/match/:matchId` | Vue arbitrage plein écran | ✅ |
| `/tournaments/:id/print` | Vue impression (planning, classements, résultats) | ✅ |
| `/settings` | Paramètres + règles de scoring custom | ✅ |
...
| Étape | Id interne | Contenu | Condition d'affichage |
|-------|-----------|---------|----------------------|
| Infos | 1 | Nom, date, lieu, nb terrains | Toujours |
| Joueurs | 2 | Sélection des participants | Toujours |
| Équipes | 3 | Répartition dans les équipes (interclub) | `teamMode = 1` uniquement |
| Format | 4 | Format de tournoi, catégories, nb poules | Toujours |
| Règles | 5 | Règle de scoring | Toujours |
| Composition | 7 | Composition des paires de doubles | Si catégories doubles présentes |
| Poules | 6 | Répartition en poules manuelles | `pool+knockout` uniquement |
...
| Format | Générateur | UI | Description |
|--------|-----------|-----|-------------|
...
| **3 · Wizard tournoi** | ✅ | Jusqu'à 7 étapes, mode interclub, composition doubles, auto-génération |
| **4 · Planning** | ✅ | Vue planning, permutation matchs, numéros de terrain, mode interclub N équipes |
| **5 · Arbitrage** | ✅ | Vue plein écran, saisie score, raccourcis clavier, avancement bracket |
| **6 · Bracket visuel** | ✅ | Onglet Bracket dans TournamentDetail (knockout, pool+knockout, labels tour/finale) |
